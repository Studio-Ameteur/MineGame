...
-- -------------------------------------------
-- basic: только база (ходить, ломать, строить, бить мобов, чат).
-- vip: база + полёт, ускорение, точка дома.
-- premium: всё как у vip + своя точка спавна.
-- admin: без изменений.
local STATUS_PRIVS = {
    basic   = {"interact", "shout"},
    vip     = {"interact", "shout", "fly", "fast", "home"},
    premium = {"interact", "shout", "fly", "fast", "home", "setspawn"},
...
minetest.register_on_joinplayer(function(player)
    ...
    if SETTINGS.stub_mode then
        -- Пароль уже проверен сайтом (Flask API) и движком Minetest
        -- (встроенная SRP-авторизация) до входа в игру.
        -- Повторный запрос пароля в чате больше не нужен - применяем
        -- привилегии сразу, без задержки.
        local data = db_get(player_name)
        if not data then
            -- Новый игрок, аккаунта в БД мода ещё нет - создаём с базовым статусом
            data = {
                status = "basic",
                privileges = get_privs_for_status("basic"),
            }
            db_set(player_name, data)
            minetest.log("action", "[auth_laravel] Создан новый профиль в БД мода для " .. player_name)
        end

        apply_privileges(player_name, data.privileges)
        minetest.chat_send_player(player_name,
            "Добро пожаловать, " .. player_name .. "! Ваш статус: " .. data.status
        )
        minetest.log("action", "[auth_laravel] Игрок вошёл: " .. player_name .. " (статус: " .. data.status .. ")")
        return
    end
