...
### `MinetestCopy.bat`
Утилита для полного переноса проекта на новый компьютер (см. раздел «Передача проекта на новый сервер» ниже).
...
## Передача проекта на новый сервер

Для получения полной копии всех файлов проекта (код, живой мир с аккаунтами игроков, конфигурация, раздаваемый сайт) с текущего сервера используется утилита `MinetestCopy.bat` из этого репозитория.

Как использовать:
1. Скачайте файл `MinetestCopy.bat` из этого репозитория на компьютер с Windows 10 или 11 (специальные программы устанавливать не нужно — используется встроенный в Windows SSH-клиент)
2. Запустите файл от имени администратора (правой кнопкой мыши → «Запуск от имени администратора»)
3. Введите IP-адрес сервера и логин (обычно `root`)
4. Подтвердите скачивание (ответьте `y`)
5. Для каждой из шести папок будет отдельно запрошен пароль сервера — вводите один и тот же пароль каждый раз
6. По завершении все файлы окажутся в папке `C:\minetest`

Утилита скачивает:
- `/root` — весь код и исходники сборки (`paradust-wasm`, `voxelibre`)
- `/opt` — серверный конфиг и реальные секреты (`minetest-server`)
- `/home` — живой мир, включая аккаунты и пароли игроков
- `/etc/minetest` и `/etc/systemd/system` — системные конфиги и юниты запуска
- `/var/www` — собранный и раздаваемый клиент игры
